...
// ── Emulation switch ──────────────────────────────────────────────────────────
//  QEMU_SIM 1  + WOKWI_SIM 0  →  QEMU mock queue, no SPI
//  QEMU_SIM 0  + WOKWI_SIM 1  →  Wokwi simulation (real SPI, 4-byte cards)
//  QEMU_SIM 0  + WOKWI_SIM 0  →  real MFRC522 over SPI
#define QEMU_SIM 0
#define WOKWI_SIM 1
...
static const uint8_t MASTER_UID[]   = { 0x04, 0x11, 0x22, 0x33 };
static const uint8_t MASTER_UID_LEN = 4;
#else
#define UID_FILTER_LEN   7
static const uint8_t MASTER_UID[]   = { 0x04, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66 };
...
// ═════════════════════════════════════════════════════════════════════════════
// REAL HARDWARE / WOKWI PATH  (QEMU_SIM == 0)
// ═════════════════════════════════════════════════════════════════════════════
#if !QEMU_SIM
...
// Burst FIFO read — one address byte then N data bytes in one SPI transaction.
// Fixes Wokwi's broken FIFO pointer on repeated single-byte reads.
// Also valid and efficient on real hardware.
...
#if WOKWI_SIM
        // Wokwi doesn't respond to SELECT — infer continuation from CT byte
        if (!has_ct) break;   // no cascade tag = last level, UID complete
#else
        uint8_t select_tx[9];
...
void app_main(void)
{
    ...
    while (cnt < 1000) {
        // ESP_LOGI(TAG, "CPU freq: %lu MHz", esp_rom_get_cpu_ticks_per_us());

        // uint32_t cycles;
        // __asm__ volatile("csrrs %0, 0x7E2, x0" : "=r"(cycles));
        // ESP_LOGI(TAG, "MPCCR direct: %lu", cycles);
        // ESP_LOGI(TAG, "esp_cpu_get_cycle_count: %lu", esp_cpu_get_cycle_count());
